...
function onInputBoxVisibleChanged(visible) {
    const inputElement = document.querySelector('div[data-testid="chat-input"]');
    if (!inputElement) {
        return false;
    }
    const textareaDom = inputElement.closest('fieldset');
    if (!textareaDom) {
        return false;
    }
...
function onSidebarVisibleChanged(visible) {
    const btn = document.querySelector('button[data-testid="pin-sidebar-toggle"]');
    if (!btn) {
        return false;
    }
    if (visible) {
...
function onAppThemeChanged(theme) {
    ...
    if (themeValue == 1 && htmlDom.dataset.mode != "light") {
        htmlDom.dataset.mode = "light";
    } else if(themeValue == 2 && htmlDom.dataset.mode != "dark") {
        htmlDom.dataset.mode = "dark";
    }
}
...
function onNewChatButtonClicked() {
    const button = a = document.querySelector('a[href="/new"]');
    if (button) {
        button.click();
    }
}
...
function onQuestionUpButtonClicked() {
    const msglist = document.querySelectorAll('div[data-testid="user-message"]');
    let list = [];
    for(let msg of msglist) {
        list.push(msg.closest('div[data-test-render-count]'));
    }
    if (list.length == 0) {
        return;
    }
    const container = list[0].closest('div[class*="scroll"]');
    if (!container) {
        return;
    }
    ...
    for(let i = 0; i < list.length; i++) {
        if (parseInt(list[i].offsetTop) >= parseInt(scrollTop)) {
            index = i - 1;
            while (index > 0 && Math.abs(list[index].offsetTop - scrollTop) < 30) {
                index -= 1;
            }
            break;
        }
...
function hookHttpsRequest() {
    ...
    window.fetch = async function (...args) {
        ...
        if (url) {
            ...
            } else if (url.match('/chat_conversations/.*rendering_mode=messages')) {
                const cloneResponse = response.clone();
                responseText = await cloneResponse.json();
...
function _parseConversationList(response) {
    ...
    for (let con of conversationList) {
        let newCon = {};
        newCon.id = con.uuid;
        newCon.title = con.name ? con.name : "Untitled";
        newCon.subTitle = con.name ? con.name : "Untitled";
